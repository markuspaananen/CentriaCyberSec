...
Note: Only mechanish to delete user by administration, seems to be on backend side.
---
...
Note: Normal user can change the reserver username and then delete someones elses reservation.
...
Note: In the logs, identificative ip addresses are collected with the logs.
---
...
Note: Might be wrong on my part, but to my knowledge, the gdpr requires the option to dowload your data from the server.
...
Note: By withdraw, in the privasypolicy, there are provided email address for this purpose.
...
Note: Privasy policy link exist, but does not show anything.
